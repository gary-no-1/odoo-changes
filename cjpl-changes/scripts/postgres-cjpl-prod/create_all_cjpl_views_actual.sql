...
CREATE OR REPLACE VIEW view_cjpl_xxx_contract_to_invoice_sale_typology AS 
 SELECT date(sale_order.date_order) AS date_order,
    sale_order.name AS so_number,
    sale_order.id AS so_id,
    sale_order.state AS sale_order_status,
    account_invoice.number AS invoice_number,
    account_invoice.id AS invoice_id,
    account_invoice.date_invoice,
    account_analytic_account.name AS contract_number,
    account_analytic_account.state AS contract_status,
...
CREATE OR REPLACE VIEW view_cjpl_xxx_direct_invoice_sale_typology AS 
 SELECT NULL::date AS date_order,
    ...
    NULL::character varying AS sale_order_status,
    account_invoice.number AS invoice_number,
    account_invoice.id AS invoice_id,
    account_invoice.date_invoice,
...
-- ------------------------------------------------------------
CREATE OR REPLACE VIEW view_cjpl_all_partners_transaction_dates AS 
 SELECT a.id,
    a.company_partner_name,
    a.company_name,
    a.min_order_date,
   ...
   FROM view_cjpl_xxx_partners_transaction_dates a
  ORDER BY a.company_name;
 ALTER TABLE view_cjpl_all_partners_transaction_dates OWNER TO postgres;
 GRANT ALL ON view_cjpl_all_partners_transaction_dates TO postgres;
 GRANT SELECT ON view_cjpl_all_partners_transaction_dates TO joomla;

-- ------------------------------------------------------------
CREATE OR REPLACE VIEW view_cjpl_invoice_service_tax_list AS 
 SELECT a.number AS invoice_number,
    a.date_invoice,
    b.company_name AS customer,
    a.amount_untaxed,
    a.amount_tax,
    a.amount_total,
    e.service_tax_base_amount,
    e.service_tax_amount,
    date(a.x_bill_period_from) AS bill_period_from,
    date(a.x_bill_period_to) AS bill_period_to,
    a.x_po_ref AS po_ref
   FROM ((account_invoice a
     JOIN ( SELECT c.invoice_id,
            max(c.base) AS service_tax_base_amount,
            sum(c.tax_amount) AS service_tax_amount
           FROM (account_invoice_tax c
             JOIN account_account d ON ((c.account_id = d.id)))
          WHERE ("position"((d.name)::text, 'Service Tax Payable'::text) > 0)
          GROUP BY c.invoice_id) e ON ((a.id = e.invoice_id)))
     LEFT JOIN view_cjpl_partner_all_companies b ON ((a.partner_id = b.id)))
  WHERE (((a.state)::text <> 'draft'::text) AND ((a.state)::text <> 'cancel'::text))
  ORDER BY a.number;
 ALTER TABLE view_cjpl_invoice_service_tax_list OWNER TO postgres;
 GRANT ALL ON view_cjpl_invoice_service_tax_list TO postgres;
 GRANT SELECT ON view_cjpl_invoice_service_tax_list TO joomla;

-- ------------------------------------------------------------
CREATE OR REPLACE VIEW view_cjpl_invoice_loss_of_assets AS 
 SELECT a.number AS invoice_number,
    a.date_invoice,
    b.company_name AS customer,
    a.amount_untaxed,
   ...
   FROM ((account_invoice a
     LEFT JOIN view_cjpl_partner_all_companies b ON ((a.partner_id = b.id)))
     LEFT JOIN view_cjpl_invoice_sale_typology c ON ((c.invoice_id = a.id)))
  WHERE ((((a.state)::text <> 'draft'::text) AND ((a.state)::text <> 'cancel'::text)) AND (("position"(upper((a.origin)::text), 'LOA'::text) > 0) OR ("position"(upper((a.origin)::text), 'LOSS'::text) > 0)))
  ORDER BY a.number;
 ALTER TABLE view_cjpl_invoice_loss_of_assets OWNER TO postgres;
 GRANT ALL ON view_cjpl_invoice_loss_of_assets TO postgres;
 GRANT SELECT ON view_cjpl_invoice_loss_of_assets TO joomla;

-- ------------------------------------------------------------
CREATE OR REPLACE VIEW view_cjpl_order_basic_with_sale_order_status AS 
 SELECT account_analytic_account.name AS contract_number,
    account_analytic_account.id,
    account_analytic_account.code,
    account_analytic_account.state AS contract_status,
        CASE
            WHEN (sale_order.project_id IS NULL) THEN 'Direct Contract'::character varying
            ELSE sale_order.state
        END AS sale_order_status,
    account_analytic_account.type,
    date(account_analytic_account.create_date) AS create_date,
    account_analytic_account.date_start AS from_date,
    account_analytic_account.date AS to_date,
    view_cjpl_partner_all_companies.company_name AS partner_name,
    view_cjpl_user_name.name AS salesperson,
    account_journal.name AS sale_journal,
    account_analytic_account.x_kind_attn AS kind_attn,
    account_analytic_account.x_po_ref AS po_ref,
    ...
    account_analytic_account.recurring_next_date
   FROM ((((account_analytic_account
     JOIN view_cjpl_partner_all_companies ON ((account_analytic_account.partner_id = view_cjpl_partner_all_companies.id)))
     JOIN view_cjpl_user_name ON ((account_analytic_account.user_id = view_cjpl_user_name.id)))
     JOIN account_journal ON ((account_journal.id = account_analytic_account.journal_id)))
     LEFT JOIN sale_order ON ((sale_order.project_id = account_analytic_account.id)))
  ORDER BY account_analytic_account.id;
 ALTER TABLE view_cjpl_order_basic_with_sale_order_status OWNER TO postgres;
 GRANT ALL ON view_cjpl_order_basic_with_sale_order_status TO postgres;
 GRANT SELECT ON view_cjpl_order_basic_with_sale_order_status TO joomla;

-- ------------------------------------------------------------
CREATE OR REPLACE VIEW view_cjpl_invoice_exam_location AS 
 SELECT a.id,
    a.invoice_id,
    a.product_id,
    c.partner_id,
    ...
    a.name AS item_description,
        CASE
            WHEN ("position"(lower((b.name_template)::text), 'okhla'::text) > 0) THEN 'Okhla'::text
            WHEN ("position"(lower((b.name_template)::text), 'naraina'::text) > 0) THEN 'Naraina'::text
            WHEN ("position"(lower(a.name), 'okhla'::text) > 0) THEN 'Okhla'::text
            WHEN ("position"(lower(a.name), 'naraina'::text) > 0) THEN 'Naraina'::text
            ELSE 'Other'::text
        END AS exam_location,
    a.quantity,
    a.price_subtotal AS price
   FROM (((account_invoice_line a
     JOIN product_product b ON ((a.product_id = b.id)))
     JOIN account_invoice c ON ((a.invoice_id = c.id)))
     JOIN view_cjpl_partner_all_companies d ON ((c.partner_id = d.id)))
  WHERE (a.invoice_id IN ( SELECT view_cjpl_invoice_sale_typology.invoice_id
           FROM view_cjpl_invoice_sale_typology
          WHERE ((view_cjpl_invoice_sale_typology.sale_type)::text = 'Exam'::text)))
  ORDER BY c.number;
 ALTER TABLE view_cjpl_invoice_exam_location OWNER TO postgres;
 GRANT ALL ON view_cjpl_invoice_exam_location TO postgres;
 GRANT SELECT ON view_cjpl_invoice_exam_location TO joomla;
