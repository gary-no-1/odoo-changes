...
class bulk_stock(osv.osv):
    _name="bulk.stock"
    def create_stock_move(self,cr,uid,ids,context=None):
        picking_obj = self.pool.get('stock.picking')
        stock_obj = self.pool.get('stock.move')
        stock1_obj = self.pool.get('bulk.stock').browse(cr,uid,ids)
        if context is None:
            context={}
        res={}
        location_list = []
...
#             picking_obj.create(cr,uid,{'move_lines':ss,'picking_type_id':list_obj.picking_type_id.id})
    ...
    _columns={
            ...
            'bulk_lines':fields.one2many('bulk.stock.line','bulk_id','Stock Lines'),
        'code':fields.boolean('Code'),
              }
class bulk_stock_line(osv.osv):
    _name="bulk.stock.line"
    _columns={
              'product_id':fields.many2one('product.product','Product'),
              'qty':fields.integer('Qty'),
              'uom_id':fields.many2one('product.uom','Uom'),
              'dest_loc':fields.many2one('stock.location','Destination Location'),
              'bulk_id':fields.many2one('bulk.stock','Bulk Lines'),
          'code':fields.boolean('Code'),
              }
    _defaults={
               'uom_id':1,
