SELECT cl_d.relname AS ref_table, a.viewname
FROM pg_rewrite AS r
INNER JOIN pg_class AS cl_r ON r.ev_class = cl_r.oid
INNER JOIN pg_depend AS d ON r.oid = d.objid
INNER JOIN pg_class AS cl_d ON d.refobjid = cl_d.oid
INNER JOIN pg_catalog.pg_views a ON cl_r.relname = a.viewname
WHERE cl_d.relkind IN ('r', 'v')
	AND (
		a.viewname LIKE '%vg%'
		OR viewname LIKE 'view_grpl%'
		)
GROUP BY cl_d.relname, a.viewname
ORDER BY a.viewname, cl_d.relname;
